...
int s21_floor_lazy(s21_decimal_lazy *value, s21_decimal_lazy *result) {
  ...
  if (!error) error |= s21_truncate_lazy(value, result);
  
  
  if (!error && result->sign && value->exponent) {
    int8_t cond = s21_is_equal_lazy(result, value);
    if (cond) error |= s21_sub_lazy(result, &lazy_one, result);
  }
